...
- **Functional Connectivity (PAC):** Quantified cross-regional communication by calculating Phase-Amplitude Coupling (PAC). Specifically, we evaluated how Hippocampal Theta phase (4-8 Hz) modulates Gamma amplitude (30-80 Hz) using the Mean Vector Length (MVL) metric across 200 ms non-overlapping sliding windows.
- **Directionality Tracking:** Deployed **Granger Causality (GC)** to explicitly test the directional flow of information, evaluating whether past hippocampal activity effectively predicts future striatal states ($HC\rightarrow dS$).
- **Statistical Evaluation:** Conducted rigorous hypothesis testing using Welch’s independent t-test to compare temporal network dynamics between standard and target (novel) auditory sequences.

---

## 🔑 Key Findings

- **Late-Latency Network Surge:** Discovered a highly robust, time-locked spike in Hippocampal-Striatal connectivity peaking between 330 and 730 ms post-stimulus. This activation closely aligns with the temporal window of the P300 cognitive component.
- **General Integration Over Selective Filtering:** Statistical analyses revealed identical coupling trajectories for both novel and standard stimuli, with no significant differences in peak PAC strength $(p=0.49)$ or Granger Causality scores $(p=0.435)$.
- **Redefining Circuit Role:** The findings challenge the assumption that the HC-dS pathway acts purely as a novelty-gated filter in passive settings. Instead, the data suggests this circuit continuously facilitates baseline orienting and temporal integration of all incoming sensory inputs.
...
- **Data Handling:** `NumPy` for managing large, multi-dimensional LFP arrays and `.npy` feature matrices.
- **Visualization:** `Matplotlib` for generating temporal trajectories, error bars, and comparative network plots].
...
- 📄 [**Robust Hippocampal-Striatal Recruitment During Auditory Processing: A Non-Selective Response to Novelty**](Neuro_Project.pdf)
...
- 📧 **Email:** [moein.yoo84@sharif.edu](mailto:moein.yoo84@sharif.edu)
